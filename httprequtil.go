...
func NewHTTPReqUtil(cfg HTTPReqUtilCfg) (*HTTPReqUtil, *AppError) {
	...
	if cfg.Method == "POST" && cfg.Payload != nil {
		...
		if reqJsonErr != nil {
			return nil, UnknownErr(reqJsonErr.Error(), op, reqJsonErr)
		}
	...
	if reqErr != nil {
		return nil, UnknownErr(reqErr.Error(), op, reqErr)
	}
...
func (svc HTTPReqUtil) Execute() (*http.Response, []byte, *AppError) {
	...
	if executeRespErr != nil {
		return nil, nil, UnknownErr(executeRespErr.Error(), op, executeRespErr)
	}
	...
	if respBodyErr != nil {
		return nil, nil, UnknownErr(respBodyErr.Error(), op, respBodyErr)
	}
